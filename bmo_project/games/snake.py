...
class SnakeGame:
    ...
    def _spawn_food(self):
        while True:
            ...
            # Avoid UI area if needed, but for snake full screen is fine
    ...
    def draw(self, screen):
        ...
        # Food
        pygame.draw.rect(screen, config.RED, (*self.food, self.grid_size, self.grid_size))
        ...
        if not self.game_over:
            hint = config.FONT_TINY.render("TAP EDGES TO TURN", True, (255, 255, 255))
        ...
        if self.game_over:
            ...
            msg = config.FONT_LARGE.render("GAME OVER", True, config.WHITE)
            hint = config.FONT_MEDIUM.render("Tap to Exit", True, config.WHITE)
